...
- **Firebase Authentication (Email/Password)**: accounts. Each username maps to
...
- **Cloud Firestore (`(default)` database, Standard edition)**: profiles, usernames, DMs, rooms,
  messages, reactions, blocks, custom emoji, read markers, push tokens, reports, suspensions and
  the mod log. Rules: `firestore.rules`. Index settings: `firestore.indexes.json`.
- **Realtime Database**: only the live bits that need clean-up when a tab closes: online status,
  typing, and voice-call setup. Rules: `database.rules.json`. Those rules can't read Firestore, so
  the `mirrorRoom`, `mirrorBlock` and `mirrorBan` functions copy room membership, blocks and
  suspensions into `mirror/` for them.
- **Voice is peer-to-peer.** Audio goes directly between browsers over WebRTC; Firebase only
  relays the short connection-setup messages.

## One-time setup (project `status-chat-12343`)

Run these from the `firebase/` folder after `npx -y firebase-tools@latest login`:

1. **Email/Password sign-in**: already configured in `firebase.json`.
   `npx -y firebase-tools@latest deploy --only auth`
   (or switch it on in the console under Authentication → Sign-in method).
2. **Firestore rules and index settings**:
   `npx -y firebase-tools@latest deploy --only firestore`
3. **Realtime Database rules**:
   `npx -y firebase-tools@latest deploy --only database`
4. **Cloud Functions** (Blaze plan):
   `(cd functions && npm install) && npx -y firebase-tools@latest deploy --only functions`
   Needed for push notifications, image scanning, report evidence, deleting a room's messages,
   and **voice and typing in rooms** (the membership mirror). DMs work without them.
5. **Cloud Vision API**: enable it for the project in Google Cloud console
   (APIs & Services → Library → "Cloud Vision API"). The scanning functions use it.
   The first 1,000 images a month are free.
6. **Moderators**: in the Firestore console create a document `mods/<uid>` (it can be empty) for
   each moderator. You can find a uid under Authentication → Users. Moderators get a shield
   button in chat. Only the console can create these.
7. Optional: set `VAPID_KEY` in `chat.js` to your Web Push certificate key
...
**The security rules are a reviewed prototype, not a guarantee.** They were tested against 25
forged-write attacks (all denied). Known gaps: nothing limits how *many* messages, custom emoji
or reports one account can create (spam volume), and anyone with a room's invite code can see
its member list (as user IDs). Review them before sharing the chat widely.
...
| Held until scanned | Uploaded images stay hidden for everyone else until the server marks them `ok`. If the server hasn't answered within 45 seconds, the in-browser result is used. The Firestore rules stop clients from marking their own images as scanned. |
...
| Blocking | Blocks are enforced by the Firestore rules (and mirrored for voice): a blocked person can't DM or call you. Their room messages are collapsed for you. |
